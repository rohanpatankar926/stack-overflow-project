...
def preparedata(config_path):
    ...
    with open(input_data,"r") as f_in:
        with open(train_data_path,"w") as train_data:
            with open(test_data_path,"w") as test_data:
                process_data(f_in=f_in.read(),target_tag="<python>",f_out_train=train_data,f_out_test=test_data,split=split_ratio)
